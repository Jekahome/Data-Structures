...
Хэш-таблицы — это одно из величайших достижений в науке программирования. В них объединены черты массивов, списков и некоторых сложных математических методов. В результате получаются эффективные структуры для хранения и обработки динамических данных.
...
> (доработать)
> Книга: Брайан Керниган, Роб Пайк. Практика программирования

Суть приёмов хеш-кодирования:
* Сведение сложных структур к простым ключам
* Преобразование многомерных данных в линейные индексы
* Обнаружение дубликатов без полного сравнения
* Сжатие состояний для экономии памяти
* Ускорение поиска за счёт предвычисленных хешей

Это именно то, о чём писали Керниган и Пайк — использование хеширования не просто для строк, а для преобразования сложных структур данных в эффективные представления.

Приёмы хеш-кодирования на примерах:

Эффект: `O(количество_частиц)` вместо `O(размер³)`:
```
// Вместо 3D массива [1000][1000][1000] = 1 млрд ячеек
// Хешируем координаты в линейный массив

int hash_3d_coord(int x, int y, int z) {
    // Простой полиномиальный хеш
    return (x * 1031 + y * 521 + z) % TABLE_SIZE;
}

// Использование:
particle_data[hash_3d_coord(x, y, z)] = particle_info;
```

Битовое хеширование состояний (Supertrace):
Эффект: Обнаружение циклов и повторяющихся состояний

```
// Хешируем состояние системы в битовый массив
uint64_t hash_state(SystemState state) {
    return compute_hash(state) % BIT_ARRAY_SIZE;
}

// Проверка уникальности состояния:
bit_index = hash_state(current_state);
if (bit_array[bit_index] == 1) {
    // Состояние уже встречалось
} else {
    // Новое состояние - устанавливаем бит
