...
**Чем полезен Binary search tree**
...
(Поиск в глубину Depth-first - На каждом шаге итератор пытается продвинуться вертикально вниз по дереву перед тем, как перейти к родственному узлу — узлу на том же уровне. Реализуется за счет использованием рекурсии, или с использованием стека)
...
**Прямой метод прохождения** `Pre-order` определяется посещением узла в первую очередь и последующим прохождением сначала левого, а потом правого его поддеревьев.
...
Прямой:	      4  3  1  2  9  7  6   8  10
Симметричный: 1  2  3  4  6  7  8   9  10
Обратный:     2  1  3  6  8  7  10  9  4
```
