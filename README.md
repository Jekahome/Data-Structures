...
Из-за динамической особенности данных поддержка бинарного дерева поиска в сбалансированном виде является необходимой. 
Необходимая высота дерева должна составлять `log N` тогда поиск будет за `O(log N)` иначе оно вырождается в список и поиск за `O(N)`.
Красно-черные деревья обеспечивают операции вставки, поиска, балансировки примерно за `O(log N)`, кроме поиска диапазона.
...
- у узла не может быть два красных потомка (операция смена цвета)
- красный узел может быть только слева (операция поворот влево при черном подителе)
...
[Red Black Tree Visualisation Tools](https://dreampuf.github.io/GraphvizOnline/)
