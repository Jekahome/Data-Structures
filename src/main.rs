...
impl<T: Ord + PartialEq + PartialOrd + Display + Clone> Tree<T> {
    ...
    unsafe fn rotate_right_item(&mut self, p_node_a: &mut NonNull<Node<T>>, parent: &mut NonNull<Node<T>>, is_left: bool) {
    ...
    unsafe fn left_rotate_item(&mut self, p_node_a: &mut NonNull<Node<T>>, parent: &mut NonNull<Node<T>>, is_left: bool){
